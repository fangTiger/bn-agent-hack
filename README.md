...
price discovery is released within roughly 5–15 minutes.
...
| Hypothesis | Result | n |
|---|---|---|
| Predict gap direction from the overnight BTC move | **50.0% hit rate**, corr +0.018 | 594 |
| Post-open momentum: does the first 30 min continue into the next 30? | gross **+0.045%**/trade (t=1.00) → **−0.155% after 0.2% fees** (t=−3.44) | 596 |

The first says direction is a coin flip. The second is the more interesting one:
**the momentum is real** (corr +0.081) — and its magnitude is one fifth of the
round-trip fee.
...
The pattern is consistent: **the inefficiency is real, and its magnitude is almost
exactly Binance's 10 bps fee.**
...
very evidence in §2; polling them would be a mistake. Repricing takes 5–15 minutes,
not milliseconds — this is a slow, calendar-driven event, which is exactly the kind
...
3. **bstock listed in June 2026.** Per-symbol history is 21–59 opening events.
   We use historical maxima with a buffer, and call the output a *stress loss*, never
   a VaR — the sample cannot support a statistical claim.
...
- **Hedging with TradFi perpetuals.** `TSLAUSDT` and ~22 other `TRADIFI_PERPETUAL`
